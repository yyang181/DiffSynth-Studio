...
class ResidualBlock2D(nn.Module):
    def __init__(self, in_dim, out_dim, dropout=0.0):
        ...
        self.residual = nn.Sequential(
            RMS_norm(in_dim, images=True),
            nn.SiLU(),
            nn.Conv2d(in_dim, out_dim, kernel_size=3, padding=1),
            RMS_norm(out_dim, images=True),
            nn.SiLU(),
            nn.Dropout(dropout),
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        ...
        v = self.v(ctx)
        # print(ctx.shape, x.shape, q.shape, k.shape, v.shape)
        x = self.attn(q, k, v)
        if self.has_image_input:
...
class WanModel_v1(torch.nn.Module):
    def __init__(
        self,
        dim: int,
        in_dim: int,
    ...
    def forward(self,
        ...
        lq_input, (f, h, w) = self.lq_patchify(lq_input)
