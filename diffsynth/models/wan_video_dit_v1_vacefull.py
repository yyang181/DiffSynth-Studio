...
class WanModel_v1_vacefull(torch.nn.Module):
    def __init__(
        self,
        dim: int,
        in_dim: int,
    ...
    def enable_vacefull_condition(self):
        # print(self.input_dim, self.dim);assert 0 # 16 1536
        self.vacefull_patch_embedding = nn.Conv3d(
            self.input_dim * 6, self.dim, kernel_size=self.patch_size, stride=self.patch_size)

    def vacefull_patchify(self, x: torch.Tensor):
        x = self.vacefull_patch_embedding(x)
        grid_size = x.shape[2:]
        x = rearrange(x, 'b c f h w -> b (f h w) c').contiguous()
        return x, grid_size  # x, grid_size: (f, h, w)
