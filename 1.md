...
CUDA_VISIBLE_DEVICES=4 torchrun --standalone --nproc_per_node=1 examples/wanvideo/model_inference/test_batch.py --checkpoint exp/train/Wan2.1-VACE-1.3B_full_4gpu
...
accelerate launch --mixed_precision=bf16 \
  ...
  --config_file ./examples/wanvideo/model_training/full/accelerate_config_1point3B.yaml\
  examples/wanvideo/model_training/train.py \
  --dataset_base_path data/example_video_dataset_sr_single \
  --dataset_metadata_path data/example_video_dataset_sr_single/metadata_vace.csv \
  --data_file_keys "video,vace_video,vace_reference_image" \
  --height 480 \
  --width 832 \
  --num_frames 49 \
  ...
  --num_epochs 1000000000000000 \
  --remove_prefix_in_ckpt "pipe.vace." \
  --output_path "./exp/train/sr_vace_pth" \
  --trainable_models "vace" \
  --extra_inputs "vace_video,vace_reference_image" \
  --use_gradient_checkpointing_offload --use_swanlab --swanlab_mode disabled --use_data_pt exp/train/data_process/data_cache
...
### data preprocess
CUDA_VISIBLE_DEVICES=0 python examples/wanvideo/model_training/train.py \
  --dataset_base_path data/example_video_dataset_sr_single \
  --dataset_metadata_path data/example_video_dataset_sr_single/metadata_vace.csv \
  --data_file_keys "video,vace_video,vace_reference_image" \
  --height 480 \
