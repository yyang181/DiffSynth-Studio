...
def run_inference(checkpoint_path):
    ...
    for i, (ref_img_path, driving_video_path, prompt) in enumerate(test_list_path):
        ...
        output_path = os.path.join(save_root, f"{video_name}_winputvideo.mp4")
        ...
        # reference_image = Image.open(ref_img_path).resize((832, 480))
        # input_video = [reference_image] + [control_video[i] for i in range(len(control_video))]
        # input_video = [control_video[i] for i in range(len(control_video))]
